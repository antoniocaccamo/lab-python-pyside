...
class BasePlayerWidget(BaseWidget):
    """
    ...
    @Slot()
    def _tick(self):
        elapsed : timedelta =  datetime.datetime.now() - self._startedAt
...
class DigitalClockWidget(BasePlayerWidget):
    """
...
class ScreenWindow(BaseWidget):
    ...
    def on_media_progress(self, progress: float):
        self.logger.info(f"{self} progress [{self._current_media}]: {progress:0.2f} / {self._current_media.duration:0.2f} ")
...
class SettingWidget(BaseWidget):
    ...
    def __init__(self, parent: QWidget, setting: Setting):
        ...
        self.window = ScreenWindow(index=self._setting.index)
    ...
    @Slot(float)
    def on_media_progress(self, progress: float):
        self.logger.info(f"{self} progress  {progress:0.2f}  ")
